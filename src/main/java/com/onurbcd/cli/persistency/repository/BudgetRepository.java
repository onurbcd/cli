...
public interface BudgetRepository extends EruRepository<Budget, BudgetDto>, SequenceRepository {
        ...
        @Query("select new com.onurbcd.cli.dto.budget.BudgetSumDto(sum(b.amount), b.paid, count(*))" +
                        " from Budget b" +
        ...
        @Query("select new com.onurbcd.cli.dto.budget.BudgetValuesDto(b.amount, b.billType.id, b.billType.path, b.paid)"
                        +
                        " from Budget b" +
                        " where b.id = :id")
        Optional<BudgetValuesDto> getBudgetValues(@Param("id") UUID id);

        @Query("select new com.onurbcd.cli.dto.budget.MonthlyBudgetDto(b.id, b.name, b.sequence)" +
                        " from Budget b" +
                        " where b.refYear = :refYear" +
                        " and b.refMonth = :refMonth" +
                        " order by b.sequence")
